...
public class FileRepositoryImpl implements FileRepositoryCustom
{
    ...
    public Pair<Long, List<File>> getByFilter(FileFilterInput input)
    {
        ...
        whereConditions.add(cb.isFalse(fileRoot.get(File_.IN_TRASH)));
        whereCountConditions.add(cb.isFalse(countRoot.get(File_.IN_TRASH)));
    ...
    public Pair<Long, List<File>> getInTrashByFilter(FileFilterInput input)
    {
        ...
        whereConditions.add(cb.isTrue(fileRoot.get(File_.IN_TRASH)));
        whereCountConditions.add(cb.isTrue(countRoot.get(File_.IN_TRASH)));
        cq.where(cb.and(whereConditions.toArray(new Predicate[]{})));
        cq = cq.orderBy(cb.desc(fileRoot.get(File_.ATTRIBUTION_DATE)));
        TypedQuery<File> query = entityManager.createQuery(cq.distinct(true));
        CountQury = CountQury.select(cb.count(countRoot));
    ...
    public Pair<Long, List<File>> getInTrashByFilterWithActivity(FileFilterInput input)
    {
        ...
        var fJoinFileActivity = fileRoot.join(File_.FILE_ACTIVITIES);
        whereConditions.add(cb.isTrue(fJoinFileActivity.get(FileActivity_.IN_TRASH)));
        whereCountConditions.add(cb.isTrue(countRoot.get(File_.IN_TRASH)));
        cq.where(cb.and(whereConditions.toArray(new Predicate[]{})));
        cq = cq.orderBy(cb.desc(fileRoot.get(File_.ATTRIBUTION_DATE)));
        TypedQuery<File> query = entityManager.createQuery(cq.distinct(true));
    ...
    public Pair<Long, List<File>> getInTrashByFilterWithTask(FileFilterInput input)
    {
        ...
        var fJoinFileActivity = fileRoot.join(File_.FILE_ACTIVITIES);
        var fJoinFileActivityTask = fJoinFileActivity.join(FileActivity_.FILE_TASKS);
        whereConditions.add(cb.isTrue(fJoinFileActivityTask.get(FileTask_.IN_TRASH)));
        whereCountConditions.add(cb.isTrue(countRoot.get(File_.IN_TRASH)));
        cq.where(cb.and(whereConditions.toArray(new Predicate[]{})));
        cq = cq.orderBy(cb.desc(fileRoot.get(File_.ATTRIBUTION_DATE)));
        TypedQuery<File> query = entityManager.createQuery(cq.distinct(true));
